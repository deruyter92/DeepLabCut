...
def _load_pose_config_defaults(
    ...
    multianimal_project = project_config.multianimalproject
    individuals = project_config.get("individuals", ["single"])
    with_identity = project_config.with_identity
    bodyparts = project_config.bodyparts
    unique_bpts = project_config.unique_bodyparts


    if net_type is None:
        # NOTE JR 2026-01-28: net_type is no longer in project config.
        # We should pass it as an argument instead.
...
def make_pytorch_pose_config(
    ...
    pose_config: DictConfig = PoseConfig().to_dictconfig()
    pose_config.metadata = project_config
    pose_config.net_type = NetType(net_type)

    # Add inference config and weight init configs
    pose_config.inference = InferenceConfig().to_dict()
    if weight_init is not None:
        weight_init: DictConfig = WeightInitialization.from_any(weight_init).to_dictconfig()
